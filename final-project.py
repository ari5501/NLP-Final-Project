...
def create_feature_vecs_and_labels(guesses_and_scores, answers, n_guesses):
    ...
    for i in range(len(answers)):
        ...
        for j in range(length):
            ...
            curr_guess_list = guesses_scores[j]
            for (guess, curr_value) in curr_guess_list:
                curr_prob.append(curr_value)
                curr_guess.append(guess)

            # getting the best answer from the guesses
            best_answer = curr_guess[np.argmax(curr_prob)]
            curr_label = 1 if best_answer == curr_answer else 0

            # appending the probabilities and labels to their corresponding lists
            prob_vec.append(curr_prob)
...
def train(args, model, train_data_loader, dev_data_loader, device):
    ...
    for idx, batch in enumerate(train_data_loader):
        ...
        output = model(question_feature_vec, question_len)
        ...
        error, num_examples = accuracy_fn(output, labels)
        ...
        if (idx+1) % args.checkpoint == 0 and idx > 0:
            ...
            dev_acc = evaluate(dev_data_loader, model, device, True)
...
#You need to write code inside this function
def evaluate(data_loader, model, device, dev=False):
    """
